...
def costars(name)
  ...
  Movie.joins(:actors).where.not(actors: {name: name}).where(movies: {id: subquery}).distinct.pluck("actors.name")
...
def actor_out_of_work
  ...
  Actor
    .select("COUNT(actors.id) AS actor_count")
    .left_outer_joins(:castings)
    .where("castings.ord IS NULL")[0]
    .actor_count
